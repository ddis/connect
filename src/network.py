...
def create_network_design(fname):
    ...
    ws = wb.active
    ws.cell(row=1, column=1, value='Бригада')
    ws.cell(row=1, column=2, value='Батальон')
    ws.cell(row=1, column=3, value='Рота')
    ws.cell(row=1, column=4, value='Взвод')
    ws.cell(row=1, column=5, value='Відділення')
    ws.cell(row=1, column=6, value='Функція')
    ws.cell(row=1, column=7, value='Назва')
    ws.cell(row=1, column=8, value='Радіостанція')
    r = 2
    ...
    for i,std_net in enumerate(std_networks):
        ws.cell(row=1, column=i+9, value=std_net)
    wb.save(fname)
    print('Saved ' + fname)
...
def setup_nets():
    ...
    os.system('start /WAIT EXCEL.EXE "' + os.getcwd() + '/' + network_design_fname + '"')
    ...
    for r in range(2,len(list(ws.rows)) + 1):
        ...
        for c in range(9,len(list(ws.columns)) + 1):
            ...
            if is_in_net:
                ...
                nets_config_db_new.setdefault(net_name, {
                    'net_name': net_name, 
                    ...
                    'key_value': ''})
                devs_config_db_new.setdefault(dev_name, {'dev_name':dev_name, 'model_name':dev_model, 'nets_names': [], 'firmware_fname':''})
                devs_config_db_new[dev_name]['nets_names'] = devs_config_db_new[dev_name]['nets_names'] + [net_name]
                print(user_name + '-' + dev_name)
        if is_in_some_net: dev_i = dev_i + 1
...
def configure_nets():
    ...
    for r,row in enumerate(ws.rows):
        ...
        for c,col in enumerate(ws.columns):
            nets_config_db_new[net_name][ws.cell(row=1, column=c+1).value] = ws.cell(row=r+1, column=c+1).value
